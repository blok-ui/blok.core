...
class RecordBuilder {
  public static function build() {
    ...
    function addProp(name:String, type:ComplexType, isOptional:Bool, isUpdateable:Bool) {
      ...
      var serializeable = Context.getType('blok.data.JsonSerializable');
      var unserializeable = Context.getType('blok.data.JsonUnserializable');
      var recordType = Context.getType('blok.data.Record');
      ...
      return macro class {
        ...
        **/
        ...
        public function hashCode():Int {
          if (__hash == null) {
            __hash = blok.core.ObjectTools.hash($v{cls.pack.concat([ cls.name ]).join('.')} + [ $a{nameBuilder} ].join(''));
          }
          return __hash;
        }
      };
