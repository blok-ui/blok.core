...
class ServiceBuilder {
  public static function build() {
    ...
    builder.addFieldMetaHandler({
      ...
      build: function (options:{}, builder, f) switch f.kind {
        case FVar(t, e):
          ...
          if (!Context.unify(t.toType(), Context.getType('blok.framework.context.ServiceProvider'))) {
            Context.error('@provide fields must be blok.framework.context.ServiceProviders', f.pos);
          }
    ...
    builder.addLater(() -> {
      ...
      builder.addFields([
...

        {
          ...
          kind: FFun({
            ...
            args: [
              { name: 'build', type: macro:(service:$ct)->blok.framework.Widget },
              { name: 'key', opt: true, type: macro:blok.framework.Key }
            ],
            expr: macro {
              return blok.framework.context.Context.use(context -> build(from(context)), null, key);
            }
          })
        }
      ]);
      ...
      macro class {
        public function register(context:blok.framework.context.Context) {
    ...
    return {
      ...
      build: function (options:{}, builder, f) switch f.kind {
        case FVar(t, e):
          ...
          //       unifies with blok.framework.context.ServiceResolver, but doing that
          ...
          useHooks.push(macro @:pos(f.pos) this.$backingName = (${path}:blok.framework.context.ServiceResolver<$t>).from(context));
        default:
  ...
  public static function buildFromField(id:String, fallback:Expr, type:ComplexType, createParams:Array<TypeParamDecl>):Field {
    return {
      ...
      kind: FFun({
        ...
        ret: type,
        args: [
          { name: 'context', type: macro:blok.framework.context.Context },
        ],
        expr: macro {
