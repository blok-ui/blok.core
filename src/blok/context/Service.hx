...
/**
  ...
  Use the `blok.context.Provider` to provide an instance of a service to
  all widgets below it in the widget tree.

  > Note: `blok.context.Service` just implements the `blok.context.ServiceProvider`
  > and the `blok.context.ServiceResolver` typedef, along with some 
  > quality-of-life features like `@use` and `@init` metadata.
  > You can create your own service without implementing `blok.context.Service`
  ...
  To run a callback after the Service is registered with `blok.context.Context`
...
**/
@:autoBuild(blok.context.ServiceBuilder.build())
interface Service {}
